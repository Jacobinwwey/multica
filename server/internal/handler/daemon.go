...
// resolveTaskWorkspaceID derives the workspace ID from issue/chat context first,
// then falls back to the task's agent workspace for manual resume tasks.
func (h *Handler) resolveTaskWorkspaceID(r *http.Request, task db.AgentTaskQueue) string {
	...
	if agent, err := h.Queries.GetAgent(r.Context(), task.AgentID); err == nil {
		return uuidToString(agent.WorkspaceID)
	}
	return ""
}
