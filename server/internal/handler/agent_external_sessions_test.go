...
func TestIsTruthyEnv(t *testing.T) {
	t.Setenv("MULTICA_ALLOW_PRIVILEGED_TOGGLE", "true")
	if !isTruthyEnv("MULTICA_ALLOW_PRIVILEGED_TOGGLE") {
		t.Fatalf("expected true for value 'true'")
	}

	t.Setenv("MULTICA_ALLOW_PRIVILEGED_TOGGLE", "1")
	if !isTruthyEnv("MULTICA_ALLOW_PRIVILEGED_TOGGLE") {
		t.Fatalf("expected true for value '1'")
	}

	t.Setenv("MULTICA_ALLOW_PRIVILEGED_TOGGLE", "false")
	if isTruthyEnv("MULTICA_ALLOW_PRIVILEGED_TOGGLE") {
		t.Fatalf("expected false for value 'false'")
	}
}

func TestResolveSelfhostComposeDir(t *testing.T) {
	tmp := t.TempDir()
	if err := os.WriteFile(filepath.Join(tmp, "docker-compose.selfhost.yml"), []byte("services: {}"), 0o644); err != nil {
		t.Fatalf("write compose file: %v", err)
	}
	if err := os.WriteFile(filepath.Join(tmp, "docker-compose.selfhost.host-codex.yml"), []byte("services: {}"), 0o644); err != nil {
		t.Fatalf("write host compose file: %v", err)
	}

	t.Setenv("MULTICA_SELFHOST_DIR", tmp)
	got, ok := resolveSelfhostComposeDir()
	if !ok {
		t.Fatalf("expected compose dir to be resolved")
	}
	if got != tmp {
		t.Fatalf("expected compose dir %q, got %q", tmp, got)
	}
}

func TestRecordHostCodexAction(t *testing.T) {
	recordHostCodexAction("enable", "docker compose ...", "ok", nil)
	last := getLastHostCodexAction()

	if last.Action != "enable" {
		t.Fatalf("expected action enable, got %q", last.Action)
	}
	if last.Command == "" {
		t.Fatalf("expected command to be set")
	}
	if last.Output != "ok" {
		t.Fatalf("expected output ok, got %q", last.Output)
	}
	if !last.UpdatedAt.After(time.Time{}) {
